...
Global Health Metrics as a field has emerged as a unified set of tools and concepts, allowing global quantification and comparison of health for every populations around the globe. % #TODO reference
If the results of key Health Metrics projects such as the Global Burden of Disease % #TODO abbr
show the strength of this approach, it is ill-suited to answer local questions or problems with which Health Systems have to contend on a daily basis. % # Develop low quality data, routine system, routine information

If their current usages do not yet allow to answer those questions, it does not mean that the tools and principles introduced by Health Metrics are not relevant to them. % #TODO analyse Murray and Frenk
